...
| Scanner adapters | Complete | Plugin classes execute bundled CLIs, capture redacted raw output, record command/exit code, expose capabilities, and normalize supported JSON outputs. | `apps/api/nope_api/scanners.py`, `apps/api/nope_api/models.py`, `apps/api/nope_api/main.py` | Broader rule-pack tuning belongs to scanner-quality iteration, not Phase 2 plumbing. | Backend scanner parser tests, `/health`, `/api/scanners/capabilities`, live ZIP scan `scan_e1b6a69758a848bd`. | None for bundled local scanner matrix. |
| Real scanner execution | Complete | API image bundles and runs Semgrep, Gitleaks, OSV-Scanner, Trivy, Checkov, Hadolint, and Bandit; live ZIP scan produced normalized findings for every scanner. | `apps/api/requirements.txt`, `apps/api/nope_api/scanners.py`, `docker/api.Dockerfile`, `security-packs/semgrep/nope.yml` | ZAP/browser crawling remains tracked under Dynamic testing, not repository scanner execution. | Backend tests, Docker build, `/health`, scanner version checks, ZIP scan `scan_e1b6a69758a848bd`. | None. |
...
- Verified MinIO object storage and DB artifact joins for all seven external scanner raw-output artifacts.

Phase 2 status: Complete for the bundled repository scanner execution and evidence pipeline.

Out of scope for Phase 2 and still tracked separately: Redis worker queue, ZAP/browser dynamic testing, Qwen inference, suppression workflow, and PDF reports.
