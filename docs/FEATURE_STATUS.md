...
| Security drift detection | Complete | Phase 8 detects new, fixed, recurring, reintroduced, severity/confidence changes, new/removed routes, new dependencies/CVEs/secrets, RLS/Supabase, CORS, headers, trackers, public assets, scanner coverage regressions, scanner status changes, stack changes, and conservative incremental scope. | `apps/api/nope_api/drift.py`, `apps/api/nope_api/main.py`, `apps/web/app/app/projects/local/scans/page.tsx` | Phase 14 still owns replacing full scans with verified incremental scans. | Phase 8 tests and live API smoke. | None. |
...
| Tests | Partially complete | Backend tests cover persistence, auth, scanner parsing/artifacts, queue execution, cancellation, retry/backoff, scan events, canonical finding dedupe/correlation, suppression expiry, recurrence, Qwen adapter failure handling, RAG, findings detail, drift, and PDF reports. | `apps/api/tests/test_pipeline.py`, `apps/api/tests/test_security.py`, `apps/api/tests/test_persistence.py`, `apps/api/tests/test_api_auth.py`, `apps/api/tests/test_scanners.py`, `apps/api/tests/test_queue.py`, `apps/api/tests/test_phase4_findings.py`, `apps/api/tests/test_phase5_qwen.py`, `apps/api/tests/test_phase6_rag.py`, `apps/api/tests/test_phase7_findings.py`, `apps/api/tests/test_phase8_drift.py`, `apps/api/tests/test_phase9_pdf_reports.py` | Sandbox/settings/E2E/security expansion remains. | `python -m pytest apps/api/tests`, frontend lint/typecheck/build, Docker smoke. | None for completed Phase 9 scope. |
