...
| Rules v2 | Works locally | 100+ rule catalog, upgraded first-party rules, context-indexed candidate generation, normalized candidate/evidence/correlation/promotion tables, owner-scoped APIs, dashboard review, reports, and Stage 13/13.5 tests. | It is bounded static analysis, not a full compiler for every language/framework. |
| Benchmarks | Passing locally and in CI for scanner-only | Scanner-only and scanner-plus-Qwen benchmarks met 41/41 expected findings, 0 FP/FN, precision/recall/F1 1.000. Repository-intelligence benchmark reached 41/41 Hit@5 locally. | Qwen benchmark requires local model/GPU runtime. |
...
| AI Investigation Engine | Works when the model is mounted | The `investigate` action produces citation-bound reports with root cause, attack flow, trust boundary, impact, fix guidance, verification steps, related findings, unknowns, history, and JSON/Markdown/PDF exports. | Investigation quality depends on deterministic evidence and retrieved context; it cannot create or promote findings. |
...
| NOPE self-security | Works locally with residuals | Auth/session/rate-limit/CSRF-origin/CORS/request-limit/container/dependency/security tests and docs. Scanner-only Python dependency graphs are isolated and audited separately. | Local Docker runner authority and upstream-pinned scanner CLI advisories remain documented production residuals. |
