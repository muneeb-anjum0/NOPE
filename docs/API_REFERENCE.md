...
- `GET /api/ai-actions/{job_id}/investigation.{format}` - export a completed investigation action as `json`, `md`, `markdown`, `pdf`, or `sarif`.
...
- Investigation actions are stored in the same durable AI job history. Every investigation statement is citation-backed and marked as Verified, Supported, Likely, Possible, or Unknown. Investigation exports include JSON, Markdown, PDF, and SARIF enrichment.
