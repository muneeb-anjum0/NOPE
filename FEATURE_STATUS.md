...
| Qwen adapter | llama.cpp config, health, graceful failure, focused finding explanation contract | Partially complete | Disabled/no-runtime path verified; llama.cpp code not inference-verified | API health and unit test | No GGUF model file found locally during search; GPU not verified | Configure model path and run AI CPU/GPU profile |
...
| Dashboard UI | Graphite landing page, routed app shell, overview, findings, attack map, coverage, scans, assets, reports, settings | Partially complete | Lint/type/build and Playwright/Edge visual pass | Web lint/type/build | Auth not implemented; some workflows still demo/local | Add auth/org/project management and deeper interactions |
