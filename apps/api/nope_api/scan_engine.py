...
async def run_repository_scan(scan: Scan, root: Path, settings: Settings) -> Scan:
    scan.status = "running"
    ...
    scan.ai_review = await run_ai_review(settings, scan.findings)
...
async def run_url_only_scan(scan: Scan, settings: Settings) -> Scan:
    scan.status = "running"
...
async def run_full_scan(scan: Scan, root: Path, settings: Settings) -> Scan:
    await run_repository_scan(scan, root, settings)
    if scan.target_url:
